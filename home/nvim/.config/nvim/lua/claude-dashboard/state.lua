...
function M.build()
  ...
  -- Batch-fetch all git branches in one shell call
  local paths = {}
  for _, session in ipairs(sessions) do
    table.insert(paths, session.path)
  end
  local branches = git.get_branches(paths)

  -- Track which panes are actually running Claude for cleanup
  local active_claude_panes = {}

  for _, session in ipairs(sessions) do
    session.branch = branches[session.path]
